...
StatsHealthBackground.Name = "Actyrn"
...
local Actyrn = UiLib:CreateWindow("bignib.VIP PRIVATE $$$$$ | Nigger sex", Vector2.new(500, 600), Enum.KeyCode.RightShift)
...
local TrashTalkTog = UtilitiesSec:AddToggle("Trash Talk", false, function(Value)
	if Value then
		local TrashTalkWords = {"gg/bignib", "gg/hUvujCnGMb", "FemboyAssPounder", "How to aim pls help", "wow my little brother was playing and beat you :rofl:", "Mobile player beat u Lol XD", "420 ping and u got SLAMMED", "ur bad", "seed", "im not locking ur just bad", "clown", "sonned", "LOLL UR BAD", "dont even try.. ur not enough for the alpha", "ez", "gg = get good", "my grandmas better than u :skull:", "hop off kid", "bro cannot aim", "u got absolutely DOGGED on", "i run this server son", "what is bro doing :skull:", "no way"}
...
RunService.Heartbeat:Connect(function()
	if AntiLock.Enabled then
		...
		local Velocity, AssemblyVelocity, Cframe = RootPart.AssemblyLinearVelocity, RootPart.AssemblyLinearVelocity, RootPart.CFrame

		if AntiLock.Mode == "Custom" then
			RootPart.AssemblyLinearVelocity = Vector3.new(AntiLock.CustomX, AntiLock.CustomY, AntiLock.CustomZ)
			RunService.RenderStepped:Wait()
			RootPart.AssemblyLinearVelocity = Velocity

		elseif AntiLock.Mode == "Prediction Changer" then
			RootPart.AssemblyLinearVelocity = Velocity * AntiLock.PredChangeAmt
			RunService.RenderStepped:Wait()
			RootPart.AssemblyLinearVelocity = Velocity
		...
		elseif AntiLock.Mode == "Up" then
			RootPart.AssemblyLinearVelocity = Vector3.new(Velocity.X, 9e9, Velocity.Z)
			RunService.RenderStepped:Wait()
			RootPart.AssemblyLinearVelocity = Velocity
		...
		elseif AntiLock.Mode == "LookVector" then
			RootPart.AssemblyLinearVelocity = Cframe.lookVector * AntiLock.LookVecAmt
			RunService.RenderStepped:Wait()
			RootPart.AssemblyLinearVelocity = Velocity

		elseif AntiLock.Mode == "AirOrthodox" then
			RootPart.AssemblyLinearVelocity = Vector3.new(777, 777, 777)
			RunService.RenderStepped:Wait()
			RootPart.AssemblyLinearVelocity = Velocity
